...
class DashboardScoreSummary(BaseModel):
    id: str
    feedback_type: str
    version_number: int | None
    total_score: int | None
    competence_scores: dict[str, int]
...
class EssayDashboardResponse(BaseModel):
    ...
    latest_score: DashboardScoreSummary | None
...
def feedback_record_to_score_dashboard_summary(
    feedback_record: FeedbackRecordDB,
    ...
    for score_item in payload.get("scores", []):
        competence = score_item.get("competence")
        score = score_item.get("score")

        if competence is not None and score is not None:
            competence_scores[competence] = score

    return DashboardScoreSummary(
        id=feedback_record.id,
        feedback_type=feedback_record.feedback_type,
        version_number=feedback_record.version_number,
...
def get_essay_dashboard(
    ...
    latest_score_record = get_latest_feedback_record(
    session=session,
    essay_id=essay_id,
    feedback_type="score",
)
    ...
    latest_full_feedback=(
        feedback_record_to_dashboard_summary(latest_full_feedback_record)
        if latest_full_feedback_record is not None
        else None
    ),
    latest_comparison=(
        feedback_record_to_dashboard_summary(latest_comparison_record)
        if latest_comparison_record is not None
        else None
    ),
    latest_score=(
        feedback_record_to_score_dashboard_summary(latest_score_record)
        if latest_score_record is not None
        else None
    ),
)
